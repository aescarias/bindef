...
|mime
|list of strings
|(required before 0.5; optional since) The media or MIME types assigned to the described format. If multiple are listed, they should be listed in descending order of preference.

|exts
|list of strings
|(required before 0.5; optional since) The file extensions commonly used for this format. If multiple are listed, they should be listed in descending order of preference. The extensions should contain a leading period.
...
|endian
|string
|(optional; added in 0.6) The byte endianness of the described format. If specified, must be exactly either "little" or "big".
|====
...
Integer types must include an `endian` key specifying either the string "little" or the string "big" exactly. This tells the runtime to read the integer in little endian or big endian byte order, respectively. An exception is made for `int8` and `uint8` where specifying a byte endianness is redundant.

The `endian` key may be specified directly in the field declaration or may be inherited from a parent structure that contains the `endian` key, which may either be the structure storing the field or the `endian` key specified in the `meta` mapping of the document.
...
Each enumeration member is a mapping that must contain the keys `ìd` and `value`. `id` is an identifier that will be used to refer to the constant value and `value` specifies the constant value itself. Consistent with format types, the keys `name` and `doc` may also be specified.
